...
public class TestSumPrimes extends TestCase  {
    ...
    public void testFourIsNotPrime() {
      SumPrimes s = new SumPrimes();
      Assert.assertFalse("check isPrime(4)", s.isPrime(4));
   }
    ...
    public void testSumOfTwo() { // 2 is prime, but not in sum
      SumPrimes s = new SumPrimes();
      Assert.assertEquals("check sumPrimes(2)", 0, s.sumPrimes(2));
    }

    public void testSumOfThree() {  // and 2 shows up here
      SumPrimes s = new SumPrimes();
      Assert.assertEquals("check sumPrimes(3)", 2, s.sumPrimes(3));
    }

    public void testSumOfFour() { // 4 is not prime but 2, 3 are prime
      SumPrimes s = new SumPrimes();
